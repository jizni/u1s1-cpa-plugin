...
面板内容：今日免费额度（带进度条）、余额（按量）、本月已用、包内剩余 Token、额度重置时间，
以及**合并后的用量包表格**（中文名、剩余/额度进度条、今日已用、适用范围、到期时间）。数字
格式与 `u1s1 usage` 的 万/亿 表达一致。

**余额口径跟官网 dashboard 对齐**：CLI 1.8.1 的 `u1s1 usage` 已不再把网关 `remaining_usd`
当“永久余额”展示（那是全部包剩余 Token 的美元折算，和用量包列表重复计数）；面板同改，
“余额(按量)”取 `bonus_balance_usd`（充值/赠送的现金余额），网关的 `remaining_usd` 保留在
JSON 里但不再渲染。网关 `/v1/me` 没有官网 `/api/me` 的 `total_usd` 累计口径，因此面板不显示
累计消费（硬造一个恒 0 的数字比不显示更误导）。

**用量包合并跟官网 dashboard 的 `mergePkgs` 对齐**（app.js）：按
`kind|scope|是否日额度|admin_grant 备注` 分组（`groupPackagesForPanel`，服务端聚合），
`count` 记合并数、`expiry_dates` 保留全部不同到期日、`has_never_expiring` 标记永不过期成员。
登录打卡每天铸一个新包（本账号 8 个 2M 包），不合并会刷屏；合并后一行显示 `×8`、16M 汇总
剩余、“最早 2026-09-27 到期 · 分批到期”。官方赠送（admin_grant）的备注是面向用户的文案，
备注不同不合并，并在行下整行展示备注。
